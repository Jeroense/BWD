...
class RefactoredTestBolOrdersController extends Controller{
        ...
        if($bolOrdersResponse['bolstatuscode'] == 200){
            ...
            if( isset($bolRespBodystdClassObject->orders[0]->orderId) && (string)$bolRespBodystdClassObject->orders[0]->orderId != '' &&  $this->nietBestaandeOrderIDSinOrderResponse == false){      // er is tenmiste 1 order, en alle orderid's zijn niet null of ''.
                ...
                foreach($bolRespBodystdClassObject->orders as $order){
                    ...
                    // dump('tot regel 99 in RefactoredTestBolOrdersController'); dump($order->orderItems);

                    $bol_single_order_resp = $this->make_V3_PlazaApiRequest('demo',  "orders/{$order->orderId}", 'get');
                    // epoch time is unix/posix time is no of sec since 1-1-1970 midnight UTC/GMT
                    ...
                    if(isset($reply_body_data['orderId']) && isset( $reply_body_data["orderItems"][0]["orderItemId"] ) ){
                        dump('op regel 110 in code');  //komt hier wel
